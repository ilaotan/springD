...
(function(){
    ...
    /*
     * 此方法获得的是插入到编辑器中的播放器地址
     * */
    function convert_H5_url(url){
    	if ( !url ) return '';
        //如果是带src的url 就截取里面的src地址
        if(url.match(/src=\"[^\s"]+/i)){
            url = url.match(/src=\"[^\s"]+/i);
            url = url[0].substr(5);
        }
        url = utils.trim(url)
        //自己写的正则 只支持优酷和腾讯的链接
        	  .replace(/static\.video\.qq\.com\/TPout.swf\?vid=([\w]+)&[\w]+/i, "v.qq.com/iframe/player.html?vid=$1&tiny=0&auto=0")
              .replace(/v\.qq\.com\/.*\.html\?vid=([\w\-=]+)/i,"v.qq.com/iframe/player.html?vid=$1&tiny=0&auto=0")
        	  .replace(/player\.youku\.com\/player\.php\/sid\/(.*)\/v\.swf/i,"player.youku.com/embed/$1")
    ...
    function convert_url(url){
        ...
        url = utils.trim(url)
            ...
            .replace(/v\.qq\.com\/.*\.html\?vid=([\w\-=]+)/i, "static.video.qq.com/TPout.swf?vid=$1")
